...
class ListTile(BorderedTile):
    def __init__(self,list_array,rect=(0,0,0,0),title=None): #n is the number of items in the list, list_array is a square bracketed array of strings
        super().__init__(rect,title)
        self.list_array=list_array
    
    def draw(self, term):
        list_y = len(self.list_array)
        list_x = 0
        for text in self.list_array:
            if len(text) > list_x:
                list_x = len(text)
        
        super().draw(term)
        text_start = (self.rect[2]-2-list_x)//2

        cursor_y_start = (self.rect[3]+1-list_y)//2
        list_idx = 0
        for text in self.list_array:
            textline=" "*(self.rect[2]-2)
            textline_array=list(textline)
            for i in range(0,len(text)):
                textline_array[i+text_start]=text[i]
            textline="".join(textline_array)
            print(term.move_xy(self.rect[0]+1,cursor_y_start+list_idx)+textline,end="",flush=True)
            list_idx = list_idx + 1
